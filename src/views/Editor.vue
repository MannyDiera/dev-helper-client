...
import { nestedToggleMachine } from '@/state-machines/toggle';
import MarkdownIt from 'markdown-it';
import { indent } from 'indent.js';
import { saveInSessionStorage, getFromSessionStorage } from '@/shared/utils';

const md = new MarkdownIt();

const savedState = JSON.parse(getFromSessionStorage('state'));
const previousState = State.create(savedState || nestedToggleMachine.initialState);
const resolvedState = nestedToggleMachine.resolveState(previousState);

export default {
  name: 'Editor',
  data: () => ({
    content: '# Markdown Editor for notes',
    toggleService: interpret(nestedToggleMachine),
    current: nestedToggleMachine.initialState
  }),
  created() {
    this.toggleService
        .onTransition((state) => {
          this.current = state;
          saveInSessionStorage('state', JSON.stringify(this.current));
        }).start(resolvedState);
  },
  computed: {
    rendered() {
